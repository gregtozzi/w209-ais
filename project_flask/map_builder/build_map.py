...
def build_map():
    ...
    lat_annotations = alt.Chart(lat_labels).encode(text='Name:N',
                                                   latitude='LAT',
                                                   longitude='LON').mark_text(angle=0, font='Roboto-Thin.ttf', color = 'grey')

    lon_labels = pd.DataFrame(lon_labels, columns=['Name', 'LAT', 'LON'])
    lon_annotations = alt.Chart(lon_labels).encode(text='Name:N',
                                                   latitude='LAT',
                                                   longitude='LON').mark_text(angle=270, font='Roboto-Thin.ttf', color = 'grey')
    water_labels = [
        ['Treas. Island', 37.839455, -122.376198, 0, 30],
        ['San Pablo Bay', 38.064760, -122.39, 0, 12],
        #['Napa River',38.115239, -122.277615,60,7],
